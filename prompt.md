...
- チケット1から順に以下の手順を繰り返し、ソフトウェアを実装する。
  - APIを介してチケットの詳細を取得する。
  - `@anthropic-ai/claude-agent-sdk`を使用して実装する。このとき同時にテストコードも実装する。初回の実装では、後続のステップで使用するコマンドを定義するよう追加で指示する。
  - `package.json`の`scripts`に`typecheck`があるならば、それを実行し、型検査を適用する。問題があれば実装ステップに戻る。
  - `package.json`の`scripts`に`format`があるならば、それを実行し、フォーマット、lintを適用する。問題があれば実装ステップに戻る。
  - `package.json`の`scripts`に`test`があるならば、それを実行し、テストを実行する。問題があれば実装ステップに戻る。
